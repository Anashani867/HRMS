      ...
      <div class="card-body" id="ajax_responce_serachDiv">
         ...
         <table id="example2" class="table table-bordered table-hover">
            ...
            <tbody>
               ...
               <tr>
                  <td>{{ $info->emp_name }}</td>
                  <td @if ($info->Functiona_status==1) class="bg-success" @else class="bg-danger"  @endif > @if ($info->Functiona_status==1) مفعل @else معطل @endif</td>
                  <td>
                     @php
                     ...
                     {{ $time }}
                     {{ $newDateTimeType }}  <br>
                     {{ optional($info->added)->name ?? 'غير معروف' }}
                  </td>
                  <td>
