   ...
   public function destroy($id)
{
    try {
        $com_code = auth()->user()->com_code;
        $data = Departement::where('com_code', $com_code)->where('id', $id)->first();

        if (!$data) {
            return redirect()->route('departements.index')->with(['error' => 'عفوا غير قادر على الوصول إلى البيانات المطلوبة!']);
        }

        DB::beginTransaction();
        $data->delete(); // سوفت دليت
        DB::commit();

        return redirect()->route('departements.index')->with(['success' => 'تم حذف البيانات بنجاح (Soft Delete)']);
    } catch (\Exception $ex) {
        DB::rollBack();
        return redirect()->back()->with(['error' => 'عفوا حدث خطأ ما ' . $ex->getMessage()])->withInput();
    }
