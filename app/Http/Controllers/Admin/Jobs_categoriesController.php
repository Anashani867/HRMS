...
class Jobs_categoriesController extends Controller
{
  public function index()
  {
    ...
    $data = get_cols_where_p(new jobs_categories(), array("*"), array('com_code' => $com_code), 'id', 'DESC', PC);
  ...
  public function store(Jobs_categoriesRequest $request)
  {
    try {
      $com_code = auth()->user()->com_code;
      $CheckExsists = get_cols_where_row(new jobs_categories(), array("id"), array("name" => $request->name, 'com_code' => $com_code));
  ...
  public function update($id, Jobs_categoriesRequest $request)
  {
    $com_code = auth()->user()->com_code;
    try {
      $CheckExsists = jobs_categories::select("id")->where("com_code", "=", $com_code)->where('name', '=', $request->name)->where('id', '!=', $id)->first();
      if (!empty($CheckExsists)) {
        return redirect()->back()->with(['error' => 'عفوا  اسم الوظيفة مسجل من قبل ! '])->withInput();
      }
      DB::beginTransaction();
